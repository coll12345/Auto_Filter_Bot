...
async def get_clone_shortlink(link, url, api):
    base = urlparse(url).netloc if url and url.startswith("http") else (url.split('/')[0] if url else url)
    shortzy = Shortzy(api_key=api, base_site=base)
...
async def get_shortlink(chat_id, link):
    ...
    if URL.startswith("shorturllink") or URL.startswith("terabox.in") or URL.startswith("urlshorten.in"):
        URL = SHORTLINK_URL
        API = SHORTLINK_API
    # normalize URL to domain only (no scheme or path) for Shortzy/Adlinkfly
    if URL:
        URL = urlparse(URL).netloc if URL.startswith("http") else URL.split('/')[0]
...
async def get_verify_shorted_link(link, url, api):
    ...
    if URL == "api.shareus.io":
        ...
        # normalize URL to domain only for Shortzy
        if URL:
            URL = urlparse(URL).netloc if URL.startswith("http") else URL.split('/')[0]
        shortzy = Shortzy(api_key=API, base_site=URL)
        link = await shortzy.convert(link)
